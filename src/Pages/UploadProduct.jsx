...
const UploadProduct = ({ userId }) => {
  ...
  const uploadPrd = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:2500/user/uploadproduct', productDetails)
    } catch (error) {
      console.log(error)
    }
  }
  ...
  return (
    <div>
      ...
      <div className="d-md-flex border">
        <div className="upload-box mt-4 pt-3 px-md-5 px-2 col-md-6 border">
          <h6 className='fs-4'><span className='text-secondary'>Upload</span> <span style={{ color: "#0DC029" }}>Products/Items</span></h6>
          <div className="p-3 img-box px-md-4">
            ...
            <div className={`p-1 inp rounded d-flex justify-content-${multipleImgArray.length == 0 ? "center align-items-center" : null}`} style={{ height: "65vh", width: "39vw", cursor: 'pointer' }}>
              {images.length > 0 ? (
                multipleImgArray.map((src, index) => (
                  <div className='border w-25 h-25 multipleImgBox' style={{ backgroundImage: `url(${src})` }}>
                    <Trash2 onClick={() => { deleteImg(src, index) }} />
        ...
        <div className="col-md-6 mt-4 pt-5">
          ...
          <textarea
            ...
            className="p-2 imgs rounded d-flex align-items-center justify-content-center"
